...
class Universe:
    ...
    def __init__(self, name: str = None, N: int = None, a: float = None, T: float = None, dim: int = 3, input_state: np.ndarray = None) -> None:
        ...
        self.compute_forces(Ewald=True)
    ...
    def compute_forces(self, Ewald: bool = False, parallel: bool = False) -> None:
        ...
        if Ewald == False:
            ...
            alpha, c_max, rbasis = init_func.ewald_parameters(rc=rc, a=self.a, N=self.N)
    ...
    def nve_integration(self, dt: float, n: int, delta: int = 1, Ewald: bool = False, parallel: bool = False, graphs: bool = True, **kwargs: bool) -> dict:
        """
    ...
    def npt_integration(self, dt: float, n: int, delta: int, T0: float, P0: float, Ewald: bool = False, graphs: bool = True, **kwargs: str) -> dict:
        ...
        Nf = lambda dim: 6 * self.N - 3 if dim == 3 else 3 * self.N - 2
        tic = time.time()
        for i in range(int(n)):
            print(f"n = {i}, t = {i * dt} fs")
            print(f"a = {self.a}")
    ...
    def nvt_integration(self, dt: float, n: int, delta: int, T0: float, Ewald: bool = False, graphs: bool = True, **kwargs: str) -> dict:
        ...
        - Ewald (bool): Utilise la méthode de sommation d'Ewald pour le calcul des interractions électrostatiques
        - graphs (bool): Trace en temps réel les variables thermodynamiques sélectionnées
        - E, H, T, P (bool): Quantités à mesurées pendant l'intégration
        """
        ...
        print(log.simulation_completed.format(time=(toc-tic)/3600))
        data['t'] = np.arange(0, n*dt, delta)
        self.__write_trajectories(dt=dt, delta=delta, a=self.a, format="vtf")
        self.__save_state_log()
        self.__save_state_variables(data=data)
        return data
