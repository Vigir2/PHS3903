...
def compute_forces_ewald(U: Universe, rc: float, a: float, alpha: float, c_max: int, rbasis: tuple):
    psi = 0
    virial = 0
...
def compute_forces_ewa(U: Universe, rc: float, a: float, alpha: float, c_max: int, rbasis: tuple):
    """
    Calcule les forces du systèmes U, l'énergie potentielle et le Viriel
...
def nve_verlet_run(U: Universe, dt: float, Ewald: bool = False) -> None:
...
def npt_verlet_run(U: Universe, T0: float, P0: float, dt: float, Nf: int) -> None:
...
if __name__ == "__main__":
    compute_forces_ewal(1, 2, 2, 1, 1, 1)
    pass
